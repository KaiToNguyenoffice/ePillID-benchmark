...
class FaissPwDistance:
    """FAISS-based pairwise distance/similarity computation.

    Uses IVF index for large-scale approximate nearest neighbor search,
    falling back to flat (exact) index for smaller reference sets.
    """
    def __init__(self, embeddings, metric='cosine', nprobe=16, use_gpu=False):
        import faiss
        ...
        if n < 4096:
            self.index = faiss.IndexFlat(dim, self._faiss_metric)
        else:
            nlist = min(int(n ** 0.5), 256)
            quantizer = faiss.IndexFlat(dim, self._faiss_metric)
            self.index = faiss.IndexIVFFlat(quantizer, dim, nlist, self._faiss_metric)
        ...
        print(f'FAISS index built: {n} vectors, dim={dim}, metric={metric}')

    def __call__(self, q_vecs):
        import faiss

        q_np = q_vecs.cpu().numpy().astype('float32')
        ...
        sim_matrix = torch.zeros(q_np.shape[0], k)
        for i in range(q_np.shape[0]):
            if self.metric_name == 'cosine':
                sim_matrix[i, indices[i]] = torch.from_numpy(distances[i])
            else:
                sim_matrix[i, indices[i]] = -torch.from_numpy(np.sqrt(np.maximum(distances[i], 0)))

        return sim_matrix

    def reset_index(self, embeddings):
        self.__init__(embeddings, metric=self.metric_name)
...
class MetricEmbeddingEvaluator(ModelEvaluator):
    ...
    def eval_model(self,
        ...
        if self.metric_evaluator_type == 'faiss':
            print("Creating FAISS dist provider for metric evaluator")
            pw_dist_provider = FaissPwDistance(ref_outputs, metric='cosine')
        elif self.metric_evaluator_type == 'ann':
