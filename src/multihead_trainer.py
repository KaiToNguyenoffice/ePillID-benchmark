...
from aml_wrapper import get_run
run = get_run()
...
def create_dataloaders(ref_only_df, cons_train_df, cons_val_df,
                       ...
                       n_samples_per_class=6, aug_backend='imgaug'):
    ...
    n_workers = 0 if sys.platform == 'win32' else 6
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for batch_index, batch_data in enumerate(pbar):
                ...
                for k in ['loss', 'metric_loss', 'ce', 'arcface', 'contrastive', 'triplet', 'focal', 'circle']:
                    if k in loss_outputs:
...
def train(ref_only_df,
    ...
    aug_backend = getattr(args, 'aug_backend', 'imgaug')
    ...
    if args.optimizer == 'momentum':
        optimizer = optim.SGD(list(model.parameters()), lr=args.init_lr, weight_decay=wd)
    elif args.optimizer == 'adamdelta':
        optimizer = optim.Adadelta(model.parameters(), lr=args.init_lr, weight_decay=wd)
    elif args.optimizer == 'adamw':
        optimizer = optim.AdamW(model.parameters(), lr=args.init_lr, weight_decay=wd if wd > 0 else 0.01)
    else:
        optimizer = optim.Adam(model.parameters(), lr=args.init_lr, weight_decay=wd)
    ...
    loss_weights = {
        'ce': args.ce_w, 'arcface': args.arcface_w,
        'contrastive': args.contrastive_w, 'triplet': args.triplet_w,
        'focal': args.focal_w,
        'circle': getattr(args, 'circle_w', 0.0),
    }
    print("loss_weights", loss_weights)
    circle_params = {
        'm': getattr(args, 'circle_m', 0.25),
        'gamma': getattr(args, 'circle_gamma', 256.0),
    }
