...
def run(args):
    ...
    from data_path_utils import validate_data_root
    args.data_root_dir = validate_data_root(args.data_root_dir)
    ...
    from label_utils import load_or_create_encoder

    label_encoder = load_or_create_encoder(
        args.label_encoder,
        all_imgs_csv=args.all_imgs_csv,
        label_col='label',
        incremental=True)

    logger.info("The label encoder has %d classes.", len(label_encoder.classes_))
